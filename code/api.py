...
@app.post("/datasets/")
def upload_file(file : UploadFile, Store_DF : list):
    global index
    ret = Dataset()
    ret.id_ds = index
    index+=1
...
def delete_ds(id : int, Store_DF : list):
    for i in range(len(Store_DF)):
        if Store_DF[i].id_ds == id:
            del(Store_DF[i])
            return 1
...
def df_to_excel(id : int, Store_DF : list):
    for i in range(len(Store_DF)):
        if Store_DF[i].id_ds == id:
...
def stats(id : int, Store_DF : list):
    for i in range(len(Store_DF)):
        if Store_DF[i].id_ds == id:
...
def get_hist(id : int, Store_DF :list):
    for i in range(len(Store_DF)):
        if Store_DF[i].id_ds == id:
